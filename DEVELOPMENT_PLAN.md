...
1. 完成导入中心交互：支持 `cmb_eml` 与 `youzhiyouxing_export` 两类来源。
2. 完成导入预览与确认入库流程（含错误反馈与任务状态）。
3. 完成投资记录单条录入页面（表单校验 + 保存 + 列表刷新）。
4. 完成有知有行导出表的列映射与批量导入。
5. 调整数据结构（重点是 `import_jobs` 与 `investment_records` 的来源追溯字段）。
6. 完成基础查询页（交易/投资记录筛选、列表、简单汇总）。
