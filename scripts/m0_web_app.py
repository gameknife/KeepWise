...
def ensure_bank_transfer_whitelist_file(config: AppConfig) -> Path:
    ...
    if missing:
        rows.extend(missing)
        rows.sort(key=lambda item: (-(1 if str(item.get("is_active", "1")) in {"1", "true", "yes", "on"} else 0), item.get("name", "")))
        write_csv_rows(path, BANK_TRANSFER_WHITELIST_HEADERS, rows)
    return path


def load_bank_transfer_whitelist_names(config: AppConfig) -> set[str]:
...
def query_bank_transfer_whitelist_rules(config: AppConfig, qs: dict[str, list[str]]) -> dict[str, Any]:
    keyword = (qs.get("keyword") or [""])[0].strip().lower()
    limit = min(max(int((qs.get("limit") or ["200"])[0] or "200"), 1), 500)
    active_only = parse_bool_param((qs.get("active_only") or ["false"])[0], default=False)
    path = ensure_bank_transfer_whitelist_file(config)
    rows = read_csv_rows(path, BANK_TRANSFER_WHITELIST_HEADERS)
...
def upsert_bank_transfer_whitelist_rule(config: AppConfig, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(raw_active, bool):
        ...
        is_active = parse_bool_param(str(raw_active), default=True)
    if not name:
...
def delete_bank_transfer_whitelist_rule(config: AppConfig, payload: dict[str, Any]) -> dict[str, Any]:
    name = str(payload.get("name", "")).strip()
    if not name:
        raise ValueError("name 必填")
    path = ensure_bank_transfer_whitelist_file(config)
...
def query_fire_progress(config: AppConfig, qs: dict[str, list[str]]) -> dict[str, Any]:
    ...
    wealth_overview = query_wealth_overview(
        config,
        {
...
class M0Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        try:
            ...
            if parsed.path == "/api/rules/category-rules":
                ...
                self._json(HTTPStatus.OK, payload)
                return
            if parsed.path == "/api/rules/bank-transfer-whitelist":
                payload = query_bank_transfer_whitelist_rules(self.config, parse_qs(parsed.query))
                self._json(HTTPStatus.OK, payload)
                return
            ...
            if parsed.path == "/api/accounts/catalog":
                ...
                self._json(HTTPStatus.OK, payload)
                return
            if parsed.path == "/api/budgets/monthly-items":
                payload = query_monthly_budget_items(self.config, parse_qs(parsed.query))
                self._json(HTTPStatus.OK, payload)
                return
            ...
            if parsed.path == "/api/analytics/wealth-curve":
                ...
                self._json(HTTPStatus.OK, payload)
                return
            if parsed.path == "/api/analytics/budget-overview":
                payload = query_budget_overview(self.config, parse_qs(parsed.query))
                self._json(HTTPStatus.OK, payload)
                return
    ...
    def do_POST(self) -> None:  # noqa: N802
        try:
            ...
            if self.path == "/api/budgets/monthly-items/upsert":
                body = self._read_json()
                payload = upsert_monthly_budget_item(self.config, body)
                self._json(HTTPStatus.OK, payload)
                return

            if self.path == "/api/budgets/monthly-items/delete":
                body = self._read_json()
                payload = delete_monthly_budget_item(self.config, body)
                self._json(HTTPStatus.OK, payload)
                return
            ...
            if self.path == "/api/admin/reset-transactions":
                body = self._read_json()
                confirm_text = str(body.get("confirm_text", "")).strip()
                clear_sessions = parse_bool_param(str(body.get("clear_import_sessions", "true")), default=True)
                payload = reset_admin_transaction_data(self.config, confirm_text=confirm_text)
                if clear_sessions:
                    payload["cleared_preview_sessions"] = self.session_store.clear_all()
                else:
                    payload["cleared_preview_sessions"] = 0
            ...
            if self.path == "/api/rules/bank-transfer-whitelist/upsert":
                body = self._read_json()
                payload = upsert_bank_transfer_whitelist_rule(self.config, body)
                self._json(HTTPStatus.OK, payload)
                return

            if self.path == "/api/rules/bank-transfer-whitelist/delete":
                body = self._read_json()
                payload = delete_bank_transfer_whitelist_rule(self.config, body)
                self._json(HTTPStatus.OK, payload)
                return
